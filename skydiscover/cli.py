...
def main() -> int:
    ...
    # Load a repo-root .env so a single file (e.g. OPENAI_API_KEY=...) configures
    # every run. override=True lets the project .env take precedence over a stale
    # global value; it only affects this process, not the machine environment.
...
def _configure_logging(level_name: Optional[str]) -> None:
    ...
    from skydiscover.search.utils.logging_utils import (
        _ConsoleFilter,
        _ConsoleFormatter,
        _force_utf8,
    )

    # Windows stdout/stderr default to cp1252; force UTF-8 so Unicode in log
    # messages doesn't raise UnicodeEncodeError before search logging is set up.
